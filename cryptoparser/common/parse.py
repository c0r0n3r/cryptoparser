...
class ParserText(ParserBase):
    ...
    def parse_separator(self, separator, min_length=1, max_length=1):
        ...
        self._parsed_length += self._check_separators('separator', self._parsed_length, separator, min_length, max_length)
    ...
    def parse_string_until_separator(self, name, separator, item_class=str):
        separator = bytearray(separator, self._encoding)
        parsed_value, parsed_length = self._parse_string_until_separator(name, self._parsed_length, separator, item_class, False)
    ...
    def parse_string_until_separator_or_end(self, name, separator, item_class=str):
        separator = bytearray(separator, self._encoding)
        parsed_value, parsed_length = self._parse_string_until_separator(name, self._parsed_length, separator, item_class, True)
    ...
    def _parse_string_array(self, name, separator, item_num=None, item_class=str):
        ...
        while item_offset < len(self._parsable):
            parsed_value, parsed_length = self._parse_string_until_separator(name, item_offset, separator, item_class, True)
    ...
    def parse_string_array(self, name, separator, item_class=str):
        value = self._parse_string_array(name, separator, item_class)
...
class ParserBinary(ParserBase):
    ...
    def parse_parsable_derived(self, name, item_base_class, fallback_class=None):
        ...
        try:
            ...
            raise InvalidValue(e.args[0], item_base_class, name)
